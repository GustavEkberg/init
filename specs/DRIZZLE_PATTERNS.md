...
### Primary Keys: Always UUIDv7

All tables **must** use UUIDv7 as primary keys, generated by PostgreSQL via the `pg_uuidv7` extension. UUIDv7 embeds a millisecond timestamp, giving natural time-ordering while remaining globally unique.

**Prerequisites:** Enable the extension in a migration before any table uses it:
...
// CORRECT - UUIDv7 primary key (DB-generated)
id: uuid('id')
  .primaryKey()
...
// WRONG - UUIDv4 (no time-ordering)
id: uuid('id').primaryKey().defaultRandom(),

// WRONG - CUID2 / text IDs (non-standard, no native UUID type)
id: text('id').primaryKey().$defaultFn(() => createId()),
```

**Exception:** Auth tables managed by better-auth (`session`, `account`, `verification`) use IDs generated by better-auth. They still use `text('id').primaryKey()` but without a default.
...
export const transaction = pgTable(
  'transaction',
  {
    // Primary key with UUIDv7 (DB-generated via pg_uuidv7 extension)
    id: uuid('id')
      .primaryKey()
      .default(sql`uuid_generate_v7()`),
    ...
    // Foreign keys (uuid type to match referenced primary keys)
...
| --------- | --------------------------------------------- | ---------- | ------------------------------ |
| UUID      | `uuid('col')`                                 | `string`   | Use for PKs and FKs            |
...
| `serial('id')` or `text('id')` for PKs | Non-standard ID strategy            | `uuid('id').default(sql\`uuid_generate_v7()\`)` |
