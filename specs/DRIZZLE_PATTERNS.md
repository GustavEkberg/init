...
### Primary Keys: Always CUID2

All tables **must** use CUID2 as primary keys. Never use auto-incrementing integers, UUIDs, or other ID strategies:

```typescript
import { createId } from '@paralleldrive/cuid2';

// CORRECT - CUID2 primary key
id: text('id')
  .primaryKey()
  .$defaultFn(() => createId()),
...
**Exception:** Auth tables managed by better-auth (`session`, `account`, `verification`) use IDs generated by better-auth. They still use `text('id').primaryKey()` but without `$defaultFn`.
...
| `serial('id')` or `uuid('id')`       | Non-standard ID strategy            | `text('id').$defaultFn(() => createId())` |
